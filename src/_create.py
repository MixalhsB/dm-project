...
with open('conditions_assignedTherapies.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
...
with open('conditions_conditionType.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
            cond, type = line.strip('\n').split(' - ')
            assert cond in conditions
            conditions[cond]['conditionType'] = type
with open('conditions_curabilityWeight.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
...
with open('conditions_sexSpecificness.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
            cond, sex = line.strip('\n').split(' - ')
...
# Therapies
therapies = {}
with open('therapies.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
            therapies[line.strip()] = {}
with open('therapies_efficacyWeight.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
...
with open('therapies_therapyType.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n':
            th, type = line.strip('\n').split(' - ')
            assert th in therapies
...
ad = AlphabetDetector()
first_names = []
with open('first_names.all.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n' and ad.only_alphabet_chars(line, "LATIN"):
            first_names.append('-'.join((x.capitalize() for x in line.strip().split('-'))))
last_names = []
with open('last_names.all.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line != '\n' and ad.only_alphabet_chars(line, "LATIN"):
...
output_filename = '_dataset.json'
with open(output_filename, 'w', encoding='utf-8') as f:
    f.write(json.dumps(result, indent = 4, ensure_ascii=False))
print('Successfully exported dataset to', output_filename)
#### Cured/uncured counter:
...
for ptc in possible_test_cases:
    if len(ptc[5]) >= 5 and len(set((x[1] for x in ptc[5][:5]))) == 5 and (len(ptc[5]) < 6 or ptc[5][4][1] != ptc[5][5][1]):
        if ptc[0].split()[0] in ('1037', '7604', '7869'): # to select three specific patients with nice example properties
            test_cases.append(ptc)
assert len(test_cases) == 3
output_filename = '_testcases.json'
with open(output_filename, 'w', encoding='utf-8') as f:
    f.write(json.dumps(test_cases, indent = 4, ensure_ascii=False))
print('Successfully exported test cases to', output_filename)
